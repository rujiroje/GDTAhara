...
const MaterialStockManager = ({ onBack }) => {
    ...
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [formData, setFormData] = useState({ materialId: '', quantity: '', lotNumber: '' });
    const [loading, setLoading] = useState(true);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await api.post('/shift-leader/stock-transactions', { ...formData, transactionType: 'IN' });
            ...
            <div className="dashboard-card">
                ...
                <div className="data-table-container">
                    <table className="data-table">
                        <thead><tr><th>วันที่/เวลา</th><th>ประเภท</th><th>จำนวน</th><th>Lot Number</th><th>ข้อมูลการผลิต</th><th>ผู้บันทึก</th></tr></thead>
                        <tbody>
    ...
    return (
        <div className="dashboard-card">
            ...
            <div className="table-header">
                ...
                <button className="add-button" onClick={() => setIsModalOpen(true)}>รับของเข้า (Stock-In)</button>
            </div>
            <div className="data-table-container">
                <table className="data-table">
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="บันทึกรับวัตถุดิบเข้าสต็อก">
...
const NgRecording = ({ onBack, activeReports }) => {
    ...
    const fetchNgLogsForReport = async (reportId) => {
        // This is a mock function for now as the backend endpoint is not ready
        console.log("Fetching logs for report (currently mocked)", reportId);
        setNgLogs([]); // Reset on new selection
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const apiCall = editingLog 
            ? api.put(`/api/ng-logs/${editingLog.id}`, formData)
            : api.post(`/shift-leader/reports/${selectedReport.id}/ng-logs`, formData);
        try {
            const { data: savedLog } = await apiCall;
            if (editingLog) {
                setNgLogs(ngLogs.map(log => log.id === editingLog.id ? savedLog : log));
                alert('แก้ไขข้อมูลสำเร็จ!');
            } else {
                setNgLogs(prevLogs => [...prevLogs, savedLog]);
    ...
    const handleDelete = async (logId) => {
        if (window.confirm('คุณแน่ใจหรือไม่ว่าต้องการลบรายการนี้?')) {
            try {
                await api.delete(`/api/ng-logs/${logId}`);
    ...
    const openEditModal = (log) => {
        setEditingLog(log);
        const ngType = ngTypes.find(nt => nt.ngDescriptionTh === log.ngType.ngDescriptionTh);
        setFormData({ ngTypeId: ngType ? ngType.id : '', quantity: log.quantity });
        setIsModalOpen(true);
    };
...
const ShiftLeaderDashboard = () => {
    ...
    if (loading) return <div className="loading-container"><h2>กำลังโหลดข้อมูล...</h2></div>;
    if (error) return <div className="dashboard-card"><p className="error-message">{error}</p><button onClick={() => setView('main')} className="back-button-sl">กลับไปเมนูหลัก</button></div>;

    // --- View Router ---
    if (view === 'detail') {
        return ( <div className="dashboard-card-sl"> <NotificationPanel /> <div className="sl-header"> <div> <h2 className="sl-title">รายละเอียด: {dashboardData.machineName}</h2> <p className="sl-subtitle">ผลิตภัณฑ์: {dashboardData.productName} | วันที่ผลิต: {dashboardData.productionDate}</p> </div> <div> <button onClick={() => setView('dashboard')} className="back-button-sl">กลับไปหน้ารวม</button> </div> </div> <ShiftDataDisplay title="กะกลางวัน (03:00 - 15:00)" data={dashboardData.dayShiftData} /> <ShiftDataDisplay title="กะกลางคืน (15:00 - 03:00)" data={dashboardData.nightShiftData} /> </div> );
    }
    if (view === 'dashboard') {
        return ( <div className="dashboard-card"> <NotificationPanel /> <div className="sl-header"> <h2 className="dashboard-title">เลือกเครื่องจักรเพื่อดู Dashboard</h2> <button onClick={() => setView('main')} className="back-button-sl">กลับไปเมนูหลัก</button> </div> {activeReports.length === 0 ? <p>ไม่มีใบสั่งผลิตที่กำลังทำงานในวันนี้</p> : <div className="report-selection-container"> {activeReports.map(report => ( <div key={report.id} className="report-card"> <h3>{report.machineName}</h3><p>{report.productName}</p><p>วันที่: {report.productionDate || report.startDate}</p><button className="select-button" onClick={() => handleSelectReport(report.id)}>ดู Dashboard</button> </div> ))} </div>} </div> );
    }
    if (view === 'stock') {
        return <LabelStockManager onBack={() => setView('main')} />;
    }
    if (view === 'material') {
        return <MaterialStockManager onBack={() => setView('main')} />;
    }
    if (view === 'ng') {
        return <NgRecording onBack={() => setView('main')} activeReports={activeReports} />;
    }
    ...
    return (
        <div className="dashboard-card">
            ...
            <div className="task-choice-container">
                <button className="task-choice-button" onClick={() => setView('dashboard')}>ดู Dashboard การผลิต</button>
                <button className="task-choice-button" style={{backgroundColor: '#e0f2fe', borderColor: '#38bdf8', color: '#0369a1'}} onClick={() => setView('material')}>จัดการสต็อกวัตถุดิบ</button>
                <button className="task-choice-button" style={{backgroundColor: '#eef2ff', borderColor: '#818cf8', color: '#4338ca'}} onClick={() => setView('stock')}>จัดการสต็อกป้าย</button>
                <button className="task-choice-button" style={{backgroundColor: '#fef2f2', borderColor: '#f87171', color: '#991b1b'}} onClick={() => setView('ng')}>บันทึกของเสีย</button>
            </div>
        </div>
    );
};
