    ...
    useEffect(() => {
        const fetchNgTypes = async () => {
            try {
                ...
                console.log('🔍 All NG Types:', response.data);
                const shiftLeaderNgTypes = response.data.filter(ng => ng.ngType === 'Shift Leader');
                console.log('🎯 Shift Leader NG Types:', shiftLeaderNgTypes);
                setNgTypes(shiftLeaderNgTypes);
                
                // หากไม่มี NG Types สำหรับ Shift Leader ให้แสดงข้อความแจ้งเตือน
                if (shiftLeaderNgTypes.length === 0) {
                    console.warn('⚠️ ไม่พบ NG Types สำหรับ Shift Leader');
                }
            } catch (error) { 
                console.error("Failed to fetch NG types", error); 
            }
        };
        fetchNgTypes();
    }, []);
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingLog ? "แก้ไขรายการของเสีย" : "บันทึกรายการของเสีย"}>
                <form onSubmit={handleSubmit}>
                    <div className="form-group"> 
                        ...
                        <select name="ngTypeId" value={formData.ngTypeId} onChange={handleFormChange} className="form-input" required> 
                            <option value="" disabled>-- เลือกประเภท --</option> 
                            {ngTypes.length > 0 ? ngTypes.map(ng => <option key={ng.id} value={ng.id}>{ng.ngDescriptionTh}</option>) : <option disabled>ไม่มีประเภทของเสียสำหรับ Shift Leader</option>}
                        </select> 
                        {ngTypes.length === 0 && (
                            <p style={{color: 'red', fontSize: '12px', marginTop: '5px'}}>
                                ⚠️ ไม่พบประเภทของเสียสำหรับ Shift Leader ในระบบ กรุณาติดต่อผู้ดูแลระบบ
                            </p>
                        )}
                    </div>
                    ...
                    <div className="form-actions"> <button type="button" onClick={() => setIsModalOpen(false)} className="cancel-button">ยกเลิก</button> <button type="submit" className="save-button" disabled={ngTypes.length === 0}>บันทึก</button> </div>
                </form>
            </Modal>
        </div>
    );
...
const ShiftLeaderDashboard = () => {
    ...
    useEffect(() => {
        const fetchActiveReports = async () => {
            setLoading(true);
            try {
                const response = await api.get('/shift-leader/active-reports');
