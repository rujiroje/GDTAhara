...
public class ProductionController {
    ...
    public ResponseEntity<ReportSummaryDto> getReportSummary(@PathVariable Long id) {
        logger.info("Fetching summary for production report with ID: {}", id);
        try {
            return ResponseEntity.ok(productionService.getReportSummary(id));
        } catch (Exception e) {
            logger.error("Error fetching report summary with ID {}: {}", id, e.getMessage(), e);
            return ResponseEntity.internalServerError().build();
        }
    }
    ...
    public ResponseEntity<DailyProductionSummaryDto> getDailySummary(
        ...
        logger.info("Fetching daily production summary for date: {}", date);
        try {
    ...
    public ResponseEntity<DailyShiftSummaryDto> getDailySummaryByShift(
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
        logger.info("Fetching daily production summary by shift for date: {}", date);
        try {
            return ResponseEntity.ok(productionService.getDailySummaryByShift(date));
        } catch (Exception e) {
