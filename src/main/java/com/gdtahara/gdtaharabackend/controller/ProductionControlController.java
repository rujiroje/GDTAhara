...
public class ProductionControlController {
    ...
    public ResponseEntity<?> getProductionReportSummary(@PathVariable Long id) {
        try {
            ...
            response.put("data", summary);
            response.put("message", "Report summary retrieved successfully");
            
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            logger.error("Error getting production report summary for ID: {}", id, e);
            
            Map<String, Object> response = new HashMap<>();
            response.put("success", false);
            response.put("message", "Error retrieving report summary: " + e.getMessage());
    ...
    /**
     * 🎯 Endpoint ที่ Frontend เรียกใช้ - เพิ่มเพื่อให้ตรงกับที่ frontend ต้องการ
     */
    ...
    public ResponseEntity<ReportSummaryDto> getReportSummary(@PathVariable Long id) {
        try {
            ...
            logger.info("Getting report summary for ID: {} by user: {}", id, username);
            
            ReportSummaryDto summary = productionService.getReportSummary(id);
            logger.info("✅ Report summary retrieved successfully for ID: {}", id);
            
            return ResponseEntity.ok(summary);
        } catch (Exception e) {
            logger.error("❌ Error getting report summary for ID: {}", id, e);
